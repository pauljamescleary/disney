...
fn run(font_path: &Path) -> Result<(), String> {
    ...
    // Interesting, query the attributes of a texture, probably difficult as this is in VRAM on the video card
    ...
    // if dst is None, the texture is STRETCHED to fill the givenr ectangle
